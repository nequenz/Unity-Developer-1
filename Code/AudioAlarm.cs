...
public class AudioAlarm : MonoBehaviour
{
    ...
    public void Play()
    {
        const float IncreaseVolumeTarget = 1f;

        SignalWithSmoothVolume( IncreaseVolumeTarget );
    }

    public void Finish()
    {
        const float DecreaseVolumeTarget = -1f;

        SignalWithSmoothVolume( DecreaseVolumeTarget );
    }
}
